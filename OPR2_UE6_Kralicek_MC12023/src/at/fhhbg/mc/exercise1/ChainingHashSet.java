...
public class ChainingHashSet {
	 ...
	 *             if the value is smaller than zero or the element already
	 *             exists
	 */
	public void insert(int val) throws ValueException {
		int hashcode = val % array.length;
		if (val < 0) {
			throw new ValueException("Elements have to be greater than zero.",
					val);
		}
	 ...
	 * @throws ValueException
	 *             if the index is smaller or bigger than the array
	 */
